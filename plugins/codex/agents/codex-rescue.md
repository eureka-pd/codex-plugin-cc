...
- The legacy `gpt-5-4-prompting` skill remains a deprecated compatibility alias; do not load it directly for new requests.
...
- If the user asks for a concrete model name such as `gpt-5.4-mini`, pass it through with `--model`; legacy model names remain pass-through values.
