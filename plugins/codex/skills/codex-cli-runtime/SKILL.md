...
- Do not use the `gpt-5-4-prompting` skill to rewrite new requests; it is retained only as a compatibility alias.
...
- `--effort`: accepted transport values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, and `ultra`.
